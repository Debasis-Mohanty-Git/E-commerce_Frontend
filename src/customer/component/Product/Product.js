...
export default function Product() {
  ...
  const handleFilter = (value, sectionId) => {
    ...
    // If the parameter exists, split the value by commas
    if (filterValue.length > 0) {
      filterValue = filterValue[0].split(",");

      if (filterValue.includes(value)) {
        // If value is already present, remove it (toggle off)
        ...
        // Value not in list, so add it (toggle on)
    ...
    // Final query update and navigation
 ...
 useEffect(() => {
  ...
  if (priceValue) {
    if (priceValue === "3999-above") {
      minPrice = 3999;
      maxPrice = 100000; // or Infinity
    } else {
              ...
              <>
                <form className="hidden lg:block">
                  <div className='flex justify-between text-center py-2'>
                    <h1 className="text-lg opacity-40 font-bold">Filter</h1>
                    <FilterListIcon />
                  </div>
